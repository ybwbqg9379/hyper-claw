...
function decodeHtmlEntities(text: string): string {
  return text.replace(/&#x([0-9A-Fa-f]+);/g, (_, hex) => String.fromCodePoint(parseInt(hex, 16)));
}
...
function formatMarkdownContent(content: string, options: Required<FormatOptions>): string {
...
export function formatMarkdown(filePath: string, options?: FormatOptions): FormatResult {
